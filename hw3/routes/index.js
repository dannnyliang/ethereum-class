...
router.post('/mintCoin', function(req, res, next) {
  bank.options.address = req.body.address;
  console.log(req.body.value, typeof req.body.value);
  bank.methods
    .mint(req.body.value)
    .send({
      from: req.body.account,
...
router.post('/transferOwner', function(req, res, next) {
  bank.options.address = req.body.address;
  console.log(req.body.to, typeof req.body.to);
  bank.methods
...
router.post('/transferTo', function(req, res, next) {
  bank.methods
    .transfer(req.body.to, req.body.value)
    .send({
      from: req.body.account,
...
// advance ether transfer function
router.post('/transferAdvTo', async function(req, res, next) {
  const gasPrice = await web3.eth.getGasPrice().then(res => res);
  ...
  web3.eth
    .sendTransaction({
      from: req.body.account,
      to: req.body.to,
      value: web3.utils.toWei(`${req.body.value}`, 'ether') - gas * gasPrice,
    })
    .on('receipt', function(receipt) {
      res.send(receipt);
    })
